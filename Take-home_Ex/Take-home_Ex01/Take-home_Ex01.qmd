...
```{r}
qa_pte <- qa_pte %>%
  mutate(Quarter = cut(`Sale Date`,
                       breaks = as.Date(c("2023-01-01", "2023-04-01", "2023-07-01", "2023-10-01", "2024-01-01")),
...
-   Private residential property transaction data from 1st January 2024 to 31st March 2024 in tibble data frame (i.e. `qa_pte`).
...
postcodes <- unique(qa_pte$`Postal Code`)
...
```{r}
qa_pte_sf <- st_as_sf(qa_pte_coor,
