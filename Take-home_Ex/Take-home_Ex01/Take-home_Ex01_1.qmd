...
write.csv(merged, file = "data/aspatial/pte_postal.csv")
...
Before performing the georelational join, one extra step is required to convert the values in `q1_pte Planning Area` column. This is because these values are made up of upper- and lowercase. On the other hand, `mpsz PLN_AREA_N` column are in uppercase.

```{r}
q1_pte <- q1_pte %>%
  mutate_at(.vars = vars(`Planning Area`), 
          .funs = list(toupper))
```

*left_join()* of **dplyr** is used to join the geographical data and attribute table using planning area name e.g. *SUBZONE_N* and *Planning Area* as the common identifier.

```{r}
mpsz_q1 <- left_join(q1_pte, mpsz,
                          by = c("Planning Area" = "SUBZONE_N"))
```

Save output in rds format

```{r}
write_rds(mpsz_q1, "data/rds/mpsz_q1.rds")
```
