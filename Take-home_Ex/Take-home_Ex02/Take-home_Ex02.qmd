...
-   Only show Y axis label i.e. "Number of Units" and "Median Price" for third plot i.e. Detached House. This will serve as an overall Y-axis label . It should be clear that this label applies to all plots in the composite plots
...
-   Show data labels for Detached House, Semi Detached House and Terrace House only
...
hist_plot_d <- hist_plot_d + geom_text(aes(label = total_units), position = position_stack(vjust = 0))
...
hist_plot_sd <- hist_plot_sd +
  ...
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        axis.title.y = element_blank()) +
   geom_text(aes(label = total_units), position = position_stack(vjust = -1))
...
hist_plot_t <- hist_plot_t +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        ...
        axis.title.y = element_blank()) +
   geom_text(aes(label = total_units), position = position_stack(vjust = 2))
```
