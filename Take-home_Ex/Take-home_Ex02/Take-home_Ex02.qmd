...
legend_data <- data.frame(Property_Type = names(prop_colors), Color = prop_colors)

# Plot a legend using ggplot2
legend_plot <- ggplot(legend_data, aes(x = "", y = "", fill = Property_Type)) +
               geom_tile() +
...
prop_price_dist <- prop_price_dist + theme(legend.position = "none")
```

Patchwork is used to create the composite plot. The legend for the plot is created separately.

```{r}
# Combine the plots
combined_plot <- hist_plot + prop_price_dist

# Set the height of the combined plot
combined_plot <- combined_plot + plot_layout(heights = c(1, 4))
