...
+-----------------------+--------------------------------------------------------------------------------------+
| cowplot               | Allows for customised arranging of plot elements                                     |
+-----------------------+--------------------------------------------------------------------------------------+

```{r}
pacman::p_load(ggrepel, patchwork, 
               ggthemes, hrbrthemes,
               tidyverse, readr, ggdist, ggridges, colorspace, knitr, forcats, reshape2, png, grid, cowplot) 
```
...
prop_price_dist <- ggplot(for_raincloud, 
  ...
  scale_fill_manual(values = prop_colors)  + 
  scale_x_discrete() + 
  labs(title = "Distribution of Transacted Price by Property Type", 
       x = "", 
       y = "Transacted Price") +
...
prop_price_dist <- ggplot(for_raincloud, 
  ...
  scale_fill_manual(values = prop_colors)  + 
  scale_x_discrete() + 
  labs(title = "Distribution of Transacted Price per Property Type", 
       x = "", 
       y = "Transacted Price") +
   theme(legend.position = "none")
...
## Transaction Price & No. of Unit sold by Property Type
...
```{r}
# Combine the plots
combined_plot1 <- hist_plot + prop_price_dist

# Set the height of the combined plot
combined_plot1 <- combined_plot1 + plot_layout(heights = c(1, 4))
...
combined_plot2 <- hist_plot_apt / hist_plot_c / hist_plot_d / hist_plot_ec / hist_plot_sd / hist_plot_t
...
-   Create overall Y-axis label "Number of Units" and "Median Price". It should be clear that this label applies to all plots in the composite plots

```{r}
# Create an overall y-axis label
pri_y <- ggdraw() + draw_label("Number of Units Sold", angle = 90, size = 10, color = "black")
sec_y <- ggdraw() + draw_label("Median Price", angle = 90, size = 10, color = "black")

# Combine the individual plots with the overall y-axis label
combined_plot3 <- pri_y | (hist_plot_apt / hist_plot_c / hist_plot_d / hist_plot_ec / hist_plot_sd / hist_plot_t) | sec_y
```

In the revised plot below, we can see that:
