...
```{r}
# Extract unique IDs from link source
id1 <- mc3_links_1_filtered2 %>%
  select(source) %>%
  distinct() %>%
  rename(id = source)

# Extract unique IDs from link target
id2 <- mc3_links_1_filtered2 %>%
  select(target) %>%
  distinct() %>%
  rename(id = target)

# Combine unique IDs from source and target, and remove duplicates
id_unique <- rbind(id1, id2) %>%
  distinct() 

# Duplicate mc3_nodes3_1
mc3_nodes4_1 <- mc3_nodes3_1

# Perform a left join with the original nodes
mc3_nodes4_1 <- left_join(id_unique, mc3_nodes4_1, by = "id")

# Add an indicator for unmatched IDs based on a column from mc3_nodes4_1
mc3_nodes4_1 <- mc3_nodes4_1 %>%
  mutate(unmatched = ifelse(is.na(founding_date), "drop", ""))

# Filter out unmatched nodes
mc3_nodes4_1 <- mc3_nodes4_1 %>%
  filter(unmatched != "drop")

# Drop column unmatched
mc3_nodes4_1 <- mc3_nodes4_1 %>%
  select(-unmatched)
```

::: callout-note
Note: 0 rows dropped, all nodes have valid links
:::

Check for NA values

```{r}
# Check for NA values in mc3_nodes4_1
colSums(is.na(mc3_nodes4_1))

# Check for NA values in mc3_links_1_filtered2
colSums(is.na(mc3_links_1_filtered2))
```

::: callout-note
There seems to be missing values in **nodes** for some entities for their Point of Contact and Head of Organisation. We will replace these values with the id.
:::

Replace missing values

```{r}
#duplicate mc3_nodes4_1
mc3_nodes5_1 <- mc3_nodes4_1

# Replace missing values in PointOfContact and HeadOfOrg with values from id
mc3_nodes5_1 <- mc3_nodes5_1 %>%
  mutate(PointOfContact = ifelse(is.na(PointOfContact), as.character(id), PointOfContact),
         HeadOfOrg = ifelse(is.na(HeadOfOrg), as.character(id), HeadOfOrg))
```

Verify changes

```{r}
# Check for NA values in mc3_nodes5_1
colSums(is.na(mc3_nodes5_1))
```

Check for Nan Values

```{r}
# Check for NaN values in mc3_nodes5_1
colSums(is.nan(as.matrix(mc3_nodes5_1[, sapply(mc3_nodes4_1, is.numeric)])))

# Check for NaN values in mc3_links_1_filtered2
colSums(is.nan(as.matrix(mc3_links_1_filtered2[, sapply(mc3_links_1_filtered2, is.numeric)])))
```
...
mc3_graph <- tbl_graph(edges = mc3_links_1_filtered2, directed = TRUE) %>%
  activate(nodes) %>%
  left_join(mc3_nodes5_1, by = c("source" = "id")) %>%
  as_tbl_graph()
```
...
```{r,eval=FALSE}
mc3_graph <- tbl_graph(nodes = mc3_nodes5_1, edges = mc3_links_1_filtered2, directed = TRUE) %>%
  mutate(betweenness_centrality = centrality_betweenness(),
         closeness_centrality = centrality_closeness())
  theme_graph()
```
