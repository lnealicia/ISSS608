...
mc3_data3$links <- subset(mc3_data3$links, select = -end_date)
```
...
```{r}
view(mc3_data3[["nodes"]])
view(mc3_data3[["links"]])
```
...
colnames(mc3_data4$nodes)
colnames(mc3_data4$links)
```
...
mc3_data4$nodes <- mc3_data4$nodes %>%
  mutate(
    founding_date = ymd(founding_date),
    last_edited_date = ymd(last_edited_date),
...
## Infer Motivations Behind Changes in Activities

Analyse centrality measures to identify key players and their activities (i.e. influential nodes):

```{r}
# Identify top nodes by betweenness centrality
top_nodes <- filtered_graph %>% 
  activate(nodes) %>% 
  as_tibble() %>% 
  filter(betweenness_centrality >= betweenness_threshold)
...
-   Include annotations for significant events (e.g., the closure of SouthSeafood Express Corp).

-   Comparing network metrics before and after key events to visualise impact.
