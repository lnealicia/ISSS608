...
```{r}
transactions_over_time <- mc3_data6$links %>%
  group_by(start_date) %>%
  summarize(count = n())
```
...
```{r,echo=FALSE}
mc3_graph <- tbl_graph(nodes = mc3_nodes2, edges = mc3_links, directed = FALSE) %>%
...
### Filtering Atypical Business Transactions

```{r}
# Filter edges for atypical business transactions
atypical_edges <- mc3_links %>%
  filter(type2 %in% c("Shareholdership", "WorksFor", "Owns.BeneficialOwnership"))
...
::: callout-note
The resulting graph needs further refinement for improved clarity. We will explore using a filter based on 99.5 percentile.
:::

### Refined Graph

Define higher centrality threshold of 99 percentile

```{r}
# Define higher centrality thresholds for more filtering
betweenness_threshold <- quantile(atypical_graph %>% 
                                    activate(nodes) %>% 
                                    pull(betweenness_centrality), 0.99)
closeness_threshold <- quantile(atypical_graph %>% 
                                  activate(nodes) %>% 
                                  pull(closeness_centrality), 0.995)
